...
def test_build_invokes_tool_with_tag_context_and_dockerfile(tmp_path):
    with patch("base.ImageBuilder.subprocess.Popen", return_value=_fake_proc(0)) as popen:
...
def test_build_returns_the_tools_nonzero_exit_code(tmp_path):
    with patch("base.ImageBuilder.subprocess.Popen", return_value=_fake_proc(7)):
...
def test_build_raises_when_the_tool_is_not_installed(tmp_path):
    with patch("base.ImageBuilder.subprocess.Popen", side_effect=FileNotFoundError):
